...
param(Name, This) ->
  ...
  case lists:keyfind(Name, 1, params(This)) of
    false ->
      no;
    {_, Value} ->
      Value
...
copy(From, To) ->
  (fs(From)):copy(path(From), path(To)).
  
copy(From, To, IfExists) ->
  (fs(From)):copy(path(From), path(To), IfExists).

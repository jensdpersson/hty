...
method_not_allowed(Okmethods, This) ->
  Htx1 = rsp_header("Allow", lists:concat(Okmethods), This),
  status(405, "Method Not Allowed", Htx1).
...
see_other(This) ->
  see_other(hty_uri:pack(path(This)), This).
see_other(URI, This) ->
  Htx1 = rsp_header("Location", URI, This),
  status(303, "See Other", Htx1).
...
temporary_redirect(URI, This) ->
  Htx1 = rsp_header("Location", URI, This),
  status(307, "Temporary Redirect", Htx1).
...
buffer(Data, This) ->
    ...
    Unread = unread(This) - case Data of
			    eos -> 0;
			    _ -> size(Data)
			end,
...
pump(SocketReader, Htx) ->
  case SocketReader:recv(unread(Htx)) of
    timeout ->
      {no, timeout};
    {done, _Reason} ->
      Htx1 = buffer(eos, Htx),
...
dispatch(Resources, This) ->
  F = fun(Resource, Htx) ->
    try
      Htx1 = ndc_push(Resource, Htx),
			io:format("dispatching to ~p~n", [Resource]),
      Htx2 = hty_resource:invoke_handle(Htx1, Resource),
			io:format("record ~p~n", [Htx2]),
      Htx3 = ndc_pop(Htx2),
      case committed(Htx3) of
        false -> {next, Htx3};
        true ->	{break, Htx3}
      end
    catch
